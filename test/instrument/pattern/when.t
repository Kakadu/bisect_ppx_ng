...
the guard, rather than the pattern.
  ...
  let _ =
  match `A `B with
  | `A (`B | `C) as ___bisect_matched_value___ when
  ...
  let _ =
  match () with
  | exception (Exit | Failure _ as ___bisect_matched_value___) when
  (((match ___bisect_matched_value___ with
  | Exit -> (___bisect_visit___ 2; ())
  | Failure _ -> (___bisect_visit___ 3; ())
  | _ -> ()))
  [@ocaml.warning "-4-8-9-11-26-27-28-33"]);
  ___bisect_post_visit___ 0 (print_endline "foo");
  true -> (___bisect_visit___ 4; ())
