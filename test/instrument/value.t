No instrumentation is inserted into expressions that are (syntactic) values.
  ...
  > EOF
  let _ = ((let rec x = 0
  and _y = 1 in x)[@ocaml.warning "-39"])
  ...
  let _ = [|0;1|]
  let _ =
  [|(___bisect_post_visit___ 0 (print_endline "foo"));(___bisect_post_visit___
  1
  (print_endline "bar"))|]
  ...
  let _ = fun () -> (___bisect_visit___ 1; print_endline "foo" : unit)
  ...
  > EOF
  let _ = (`Foo :> [ `Foo  | `Bar ])
  let f () = ___bisect_visit___ 0; `Foo
  let _ = (___bisect_post_visit___ 1 (f ()) :> [ `Foo  | `Bar ])
  let _ = fun () -> ___bisect_visit___ 2; (f () :> [ `Foo  | `Bar ])
  ...
  > EOF
  module type X  = sig val x : unit end
  let _ = ((module struct let x = () end) : (module X))
  let _ = ((module
  struct let x = ___bisect_post_visit___ 0 (print_endline "foo") end) :
  (module X))
  ...
  [@@@ocaml.warning "-33"]
  let _ = let open List in ignore
  let _ = let open List in ___bisect_post_visit___ 0 (print_endline "foo")
  let _ =
  fun () -> ___bisect_visit___ 1; (let open List in print_endline "foo")
